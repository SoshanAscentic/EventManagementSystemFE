...
export const HomePage = () => {
  ...
  const isAdmin = hasPermission('canViewAnalytics') || hasPermission('canAccessAdminPanel')

  // Track whether to show entrance animations (only on first load)
  const [showAnimations, setShowAnimations] = useState(false)
  const hasCheckedAnimations = useRef(false)

  // Animation tracking logic - same as EventsPage
  useEffect(() => {
    ...
    const hasVisitedBefore = sessionStorage.getItem('homepage-visited') === 'true'
  ...
  // Get dashboard statistics for total users count - ONLY for admins
  const { 
    data: dashboardStatsData, 
    isLoading: dashboardLoading,
    error: dashboardError
  } = useGetDashboardQuery(undefined, {
    skip: !isAdmin, // Skip this query if user is not an admin
    refetchOnMountOrArgChange: true
  })

  // Fallback: Get users data if dashboard fails (for total count) - ONLY for admins
  const { 
    data: usersData, 
    ...
    skip: !isAdmin || !dashboardError, // Skip if not admin OR if dashboard didn't fail
    refetchOnMountOrArgChange: true
  })
  ...
  // Loading state - don't wait for admin data if user is not admin
  const isLoadingUsers = isAdmin ? (dashboardLoading || (dashboardError && usersLoading)) : false
    ...
    <div className="min-h-screen">
      ...
      <section className="relative min-h-screen flex items-center justify-center overflow-hidden bg-white">
        ...
        <div className="absolute inset-0">
          ...
          {/* Dot pattern with animations */}
          <div className="absolute top-0 left-0 w-full h-full opacity-30">
            <div className={`absolute top-1/4 left-1/4 w-2 h-2 bg-blue-400 rounded-full ${getAnimationClass('animate-fade-in')}`} style={getAnimationStyle('1s')}></div>
            <div className={`absolute top-1/3 right-1/3 w-1 h-1 bg-indigo-400 rounded-full ${getAnimationClass('animate-fade-in')}`} style={getAnimationStyle('1.2s')}></div>
            <div className={`absolute bottom-1/3 left-1/2 w-1.5 h-1.5 bg-violet-400 rounded-full ${getAnimationClass('animate-fade-in')}`} style={getAnimationStyle('1.4s')}></div>
            <div className={`absolute bottom-1/4 right-1/4 w-2 h-2 bg-blue-300 rounded-full ${getAnimationClass('animate-fade-in')}`} style={getAnimationStyle('1.6s')}></div>
          </div>
        </div>
          ...
          <div className="max-w-6xl mx-auto">
            <div className="grid lg:grid-cols-2 gap-12 items-center">
              ...
              <div className="space-y-8">
                ...
                <div className={`inline-flex items-center px-4 py-2 bg-blue-50 border border-blue-100 rounded-full text-sm font-medium text-blue-700 ${getAnimationClass('animate-slide-down')}`} style={getAnimationStyle('0s')}>
                ...
                <div className="space-y-4">
                  <h1 className={`text-4xl lg:text-6xl font-bold text-gray-900 leading-tight ${getAnimationClass('animate-slide-down')}`} style={getAnimationStyle('0.1s')}>
                    Create & Manage
                    <span className="relative inline-block ml-3">
                      ...
                      <div className={`absolute -bottom-2 left-0 w-full h-1 bg-gradient-to-r from-blue-400 to-violet-400 rounded-full ${getAnimationClass('animate-slide-width')}`} style={getAnimationStyle('0.3s')}></div>
                    </span>
                    <br />
                    That Matter
                  </h1>
                  <p className={`text-xl text-gray-600 leading-relaxed max-w-2xl ${getAnimationClass('animate-slide-up')}`} style={getAnimationStyle('0.2s')}>
                ...
                <div className={`flex flex-col sm:flex-row gap-4 ${getAnimationClass('animate-slide-up')}`} style={getAnimationStyle('0.3s')}>
                  <Button 
                    size="lg" 
                ...
                <div className={`flex flex-col sm:flex-row items-start sm:items-center space-y-4 sm:space-y-0 sm:space-x-6 pt-4 ${getAnimationClass('animate-fade-in')}`} style={getAnimationStyle('0.4s')}>
                  {isAdmin && (
              ...
              <div className={`relative ${getAnimationClass('animate-slide-up')}`} style={getAnimationStyle('0.2s')}>
                <div className="relative">
                    ...
                    <div className="p-6 space-y-4">
                      ...
                      <div className="grid grid-cols-2 gap-3">
                        ...
                        {isAdmin ? (
                          ...
                          <div className="bg-green-50 rounded-lg p-3">
                            <div className="text-2xl font-bold text-green-600">{statistics.totalCategories}</div>
                            <div className="text-xs text-green-500">Categories</div>
                          </div>
                        )}
                      </div>
                  ...
                  <div className={`absolute -top-4 -right-4 w-16 h-16 bg-gradient-to-br from-violet-400 to-purple-500 rounded-xl flex items-center justify-center shadow-lg transform -rotate-12 hover:rotate-0 transition-transform duration-300 ${getAnimationClass('animate-fade-in')}`} style={getAnimationStyle('0.5s')}>
                    <Icon name="TrendingUp" className="w-8 h-8 text-white" />
                  </div>
                  
                  <div className={`absolute -bottom-6 -left-6 w-20 h-12 bg-gradient-to-br from-emerald-400 to-green-500 rounded-lg flex items-center justify-center shadow-lg transform rotate-6 hover:rotate-3 transition-transform duration-300 ${getAnimationClass('animate-fade-in')}`} style={getAnimationStyle('0.6s')}>
                    <Icon name="Users" className="w-6 h-6 text-white" />
                  </div>
                  
                  {/* Notification Badge with animation */}
                  <div className={`absolute top-8 -left-4 bg-white rounded-lg shadow-lg p-3 border border-gray-100 transform -rotate-6 hover:-rotate-3 transition-transform duration-300 ${getAnimationClass('animate-slide-up')}`} style={getAnimationStyle('0.7s')}>
        ...
        <div className={`absolute bottom-8 left-1/2 transform -translate-x-1/2 animate-bounce ${getAnimationClass('animate-fade-in')}`} style={getAnimationStyle('1s')}>
      ...
      <section className="py-20 bg-gradient-to-br from-white via-blue-50/20 to-indigo-50/30 relative overflow-hidden">
        ...
        <div className="absolute inset-0">
          ...
          <div className={`absolute top-10 right-10 w-40 h-40 bg-gradient-to-br from-blue-100 to-indigo-200 rounded-full opacity-20 ${getAnimationClass('animate-float')}`}></div>
          <div className={`absolute bottom-20 left-20 w-32 h-32 bg-gradient-to-br from-violet-100 to-purple-200 rounded-2xl rotate-45 opacity-20 ${getAnimationClass('animate-float-delay-3')}`}></div>
        </div>
        
        <div className="relative container mx-auto px-4">
          <div className="text-center mb-16">
            <div className={`inline-flex items-center px-4 py-2 bg-blue-50 border border-blue-100 rounded-full text-sm font-medium text-blue-700 mb-6 ${getAnimationClass('animate-fade-in')}`} style={getAnimationStyle('0.1s')}>
              <Icon name="TrendingUp" className="w-4 h-4 mr-2" />
              Real-time Platform Metrics
            </div>
            <h2 className={`text-3xl lg:text-4xl font-bold text-gray-900 mb-4 ${getAnimationClass('animate-slide-down')}`} style={getAnimationStyle('0.2s')}>
              Platform 
              <span className={`bg-gradient-to-r from-blue-600 via-violet-600 to-indigo-600 bg-clip-text text-transparent ml-2 ${getAnimationClass('animate-gradient')}`}>
                Statistics
              </span>
            </h2>
            <p className={`text-lg text-gray-600 ${getAnimationClass('animate-slide-up')}`} style={getAnimationStyle('0.3s')}>
              See how our community is growing and thriving
            </p>
          </div>
          
          {/* Dynamic Stats Grid with staggered animations */}
          <div className={`grid grid-cols-1 md:grid-cols-2 ${isAdmin ? 'lg:grid-cols-4' : 'lg:grid-cols-3'} gap-8`}>
            {/* Total Events Card */}
            <div className={`${getAnimationClass('animate-slide-up')}`} style={getAnimationStyle('0.4s')}>
            ...
            {/* Participants Card - Only show for admins */}
            {isAdmin && (
              <div className={`${getAnimationClass('animate-slide-up')}`} style={getAnimationStyle('0.5s')}>
                <div className="group relative bg-white/80 backdrop-blur-sm rounded-2xl shadow-lg hover:shadow-2xl transition-all duration-500 hover:-translate-y-2 border border-white/20 overflow-hidden">
            ...
            {/* Categories Card */}
            <div className={`${getAnimationClass('animate-slide-up')}`} style={getAnimationStyle(isAdmin ? '0.6s' : '0.5s')}>
              <div className="group relative bg-white/80 backdrop-blur-sm rounded-2xl shadow-lg hover:shadow-2xl transition-all duration-500 hover:-translate-y-2 border border-white/20 overflow-hidden">
            ...
            {/* Active Events Card */}
            <div className={`${getAnimationClass('animate-slide-up')}`} style={getAnimationStyle(isAdmin ? '0.7s' : '0.6s')}>
              <div className="group relative bg-white/80 backdrop-blur-sm rounded-2xl shadow-lg hover:shadow-2xl transition-all duration-500 hover:-translate-y-2 border border-white/20 overflow-hidden">
      ...
      <section className="py-20 bg-gradient-to-br from-gray-50 to-blue-50/50">
        <div className="container mx-auto px-4">
          <div className="text-center mb-16">
            <h2 className={`text-3xl lg:text-4xl font-bold text-gray-900 mb-4 ${getAnimationClass('animate-slide-down')}`} style={getAnimationStyle('0.1s')}>Featured Events</h2>
            <p className={`text-lg text-gray-600 max-w-2xl mx-auto ${getAnimationClass('animate-slide-up')}`} style={getAnimationStyle('0.2s')}>
          ...
          {featuredEvents.length > 0 ? (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              {featuredEvents.map((event, index) => (
                <div key={event.id} className={`${getAnimationClass('animate-slide-up')}`} style={getAnimationStyle(`${0.3 + index * 0.1}s`)}>
                  <EventCard 
                    event={event}
                    showActions={true}
            ...
            <div className={`text-center py-12 ${getAnimationClass('animate-slide-up')}`} style={getAnimationStyle('0.3s')}>
          ...
          {featuredEvents.length > 0 && (
            <div className={`text-center mt-16 ${getAnimationClass('animate-slide-up')}`} style={getAnimationStyle(`${0.4 + featuredEvents.length * 0.1}s`)}>
      ...
      <section className="py-20 bg-white">
        <div className="container mx-auto px-4">
          <div className="text-center mb-16">
            <h2 className={`text-3xl lg:text-4xl font-bold text-gray-900 mb-4 ${getAnimationClass('animate-slide-down')}`} style={getAnimationStyle('0.1s')}>Browse by Category</h2>
            <p className={`text-lg text-gray-600 ${getAnimationClass('animate-slide-up')}`} style={getAnimationStyle('0.2s')}>
          ...
          {categoriesWithCounts.length > 0 ? (
            <>
              <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-6">
                {categoriesWithCounts.map((category, index) => (
                  <Link
                    ...
                    className={`group ${getAnimationClass('animate-slide-up')}`}
                    style={getAnimationStyle(`${0.3 + index * 0.1}s`)}
                  >
              ...
              <div className={`text-center mt-16 ${getAnimationClass('animate-slide-up')}`} style={getAnimationStyle(`${0.4 + categoriesWithCounts.length * 0.1}s`)}>
                <Button variant="outline" size="lg" className="hover-lift shadow-md bg-white" asChild>
                  <Link to="/categories">
                    View All Categories
                    <Icon name="ArrowRight" className="ml-2 h-4 w-4" />
      ...
      <section className="py-20 bg-gray-50">
        <div className="container mx-auto px-4">
          <div className="text-center mb-16">
            <h2 className={`text-3xl lg:text-4xl font-bold text-gray-900 mb-4 ${getAnimationClass('animate-slide-down')}`} style={getAnimationStyle('0.1s')}>Why Choose EventHub?</h2>
            <p className={`text-lg text-gray-600 max-w-2xl mx-auto ${getAnimationClass('animate-slide-up')}`} style={getAnimationStyle('0.2s')}>
            ...
            <div className={`text-center ${getAnimationClass('animate-slide-up')}`} style={getAnimationStyle('0.4s')}>
              ...
              <p className="text-gray-600 leading-relaxed">
                {isAdmin 
                  ...
                  : "Join a vibrant community of event enthusiasts and expand your professional network."
                }
              </p>
            </div>
            
            <div className={`text-center ${getAnimationClass('animate-slide-up')}`} style={getAnimationStyle('0.5s')}>
      ...
      <style>{`
        ...
        .animate-slide-width { animation: slide-width 0.6s ease-out forwards; width: 0; }
        .animate-gradient { 
